...
<table width="190" border="0" align="center" cellpadding="1" cellspacing="0">
        ...
        <%     		User user = (User)trainer;
        			...
        			if (client != null)
                        ...
                        long endTimeFilter = startTimeFilter + 24*60*60*1000;
               		Vector appointments = user.getAppointments();
               		int count = 0;               		               		
               		Enumeration appointmentList = appointments.elements();
               		while (appointmentList.hasMoreElements())
               		{
               			Appointment a = (Appointment)appointmentList.nextElement();
               			long startTime = a.getStartTime();
               			long endTime = a.getEndTime();
                                if ( (endTime < startTimeFilter) || (startTime > endTimeFilter) ) {
                                    continue;
                                }
                                
               			String timeDisplay = SMUtils.getTime(startTime, startTimeFilter) + " - " + SMUtils.getTime(endTime, startTimeFilter);
               			String color = "#FFCC66";
               			if ( ((count++)%2) == 0)
               				color = "#FFFFFF";
               			String linkDisplay = "";
                                
                                Enumeration userLinks = a.getUserAppointments().elements();
               		...
               		%>
        <tr> 
          <td width="95" bgcolor="<%=color%>"><font size="1" face="Arial, Helvetica, sans-serif"><%=timeDisplay%></font></td>
          <td width="95" bgcolor="<%=color%>"><div align="right"><a href="apptDetail.jsp?id=<%=a.getID()%>"><font size="1" face="Arial, Helvetica, sans-serif"><%=linkDisplay%></font></a></div></td>
          <%
               		}		
               %>
        </tr>
