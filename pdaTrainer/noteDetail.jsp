...
<%
	...
	Note note = client.getNote(Integer.parseInt(id));
        String action = request.getParameter("action");
        if (action == null)
            action = "";
        if (action.equals("makeInactive")) {
	...
	String creator = "";
	User user = note.getCreator();
	creator = user.getFirstName() + " " + user.getLastName();
	String date = "";
	GregorianCalendar gc = new GregorianCalendar();
	gc.setTimeInMillis(note.getDate());
	date += SMUtils.getMonth(gc.get(Calendar.MONTH)) + " " + gc.get(Calendar.DAY_OF_MONTH) + ", " + gc.get(Calendar.YEAR);
%>
<html>
<head>
...
<!-- fwtable fwsrc="newPDADesign.png" fwbase="pda2.gif" fwstyle="Dreamweaver" fwdocid = "742308039" fwnested="0" -->
  ...
  <tr>
    ...
    <td colspan="4" valign="top"><table width="90%" border="0" align="center" cellpadding="1" cellspacing="0">
        ...
        <tr> 
          <td colspan="2" bgcolor="#3366CC"><div align="center"><font color="#FFFFFF" size="2" face="Arial, Helvetica, sans-serif"><strong><%=note.getSubject()%></strong></font></div></td>
        </tr>
        <tr> 
          <td colspan="2" bgcolor="#3366CC"><div align="center"><font color="#FFFFFF" size="2" face="Arial, Helvetica, sans-serif"><strong><%=date%></strong></font></div></td>
        </tr>
        <tr> 
          <td colspan="2"><font size="2" face="Arial, Helvetica, sans-serif"><strong><%=note.getNote()%></strong></font></td>
        </tr>
        <tr> 
          <td colspan="2" bgcolor="#3366CC"><div align="center"><font color="#FFFFFF" size="1" face="Arial, Helvetica, sans-serif"><strong>Created 
              By: <%=creator%></strong></font></div></td>
        </tr>
        <tr> 
          <td><a href="viewNotes.jsp"><img src="buttons/back.gif" width="35" height="19" border="0"></a></td>
<%
    if (note.getCreator().getID() == trainer.getID()) {%>
          <td><a href="noteDetail.jsp?action=makeInactive&id=<%=note.getID()%>">
            <div align="right"><img src="buttons/makeInactive.gif" width="83" height="19" border="0"></div>
            </a></td>
<%  }%>
        </tr>
      </table></td>
