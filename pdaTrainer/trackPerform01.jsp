          ...
          <td><strong><font face="Arial, Helvetica, sans-serif"> 
            <form method="post" name="routines" action="">
              <select name="routine" size="9">
				...
				Enumeration clientRoutines = client.getRoutines().elements();
                  		while (clientRoutines.hasMoreElements())
                  		{
                  			Routine currentClientRoutine = (Routine)clientRoutines.nextElement();
                  			String routineName = currentClientRoutine.getName();
                  			...
                  			{                                                
	                  			Routine ttemp = (Routine)wsRoutines.nextElement();
                                                System.out.println("\t" + ttemp.getName());
	                  			if (ttemp.getName().equals(routineName))
	                  			{
						...
						Enumeration crExercises = temp.getExercises().elements();
						while (crExercises.hasMoreElements())
						{
                                                        System.out.println("Excercise");
							totalExcercises++;
							if ( ((Exercise)crExercises.nextElement()).getDone() )
								completeExcercises++;
						}
                  				routineName += " " + completeExcercises + "/" + totalExcercises;
                  			}
                  			out.println("<option value=\"" + currentClientRoutine.getID() + "\">" + routineName + "</option>");
                  		}
                  %>
              </select>
              <br>
